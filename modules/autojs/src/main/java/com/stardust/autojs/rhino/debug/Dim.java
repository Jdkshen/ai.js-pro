...
public class Dim {
    ...
    private String loadSource(String sourceUrl) {
        String source = null;
        // Rhino uses pseudo source names such as <init>, <stdin> and <eval> for
        // generated bootstrap code. They are not URLs and must never be sent to
        // Android's URL stack (for example, "http://<init>/" is an invalid URI).
        if (sourceUrl == null || (sourceUrl.startsWith("<") && sourceUrl.endsWith(">"))) {
            return null;
        }
