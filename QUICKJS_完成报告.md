...
> 核对日期：2026-09-01；本报告汇总 QuickJS 引擎从首批能力到全模块白名单桥的实现状态、测试结果和后续方向。
...
2. **.so 同步**：`build-quickjs.ps1` 会自动把 `libquickjs.so` 和 `libquickjs_jni.so` 复制到三个 ABI 的 `autojs/src/main/jniLibs/`；成功后再执行 Gradle 打包
