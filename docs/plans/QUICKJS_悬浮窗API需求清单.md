...
> 回归：QuickJS 全模块回归 **228 项 全绿**（批A/B/C + 用户的 P1-1/P2-1/P2-3 补丁共 +14 项悬浮窗断言，圆形触摸穿透 +7 项，批F 反馈修复 +2 项，`app` 版本成员 +3 项）。
>
> `app` 模块补齐 `versionCode` / `versionName` / `autojs`（Rhino `__app__.js` 里有、QuickJS 之前缺；脚本用 `app.versionName` 会拿到 undefined）。
