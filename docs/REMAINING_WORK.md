...
1. **MCP 收口**：确定“手机确认”或“免审批”并统一代码、页面、文档和测试；
2. **发布基线**：签名、权限、release 构建、版本和升级测试；
3. **UI 验收**：搜索定位、弹层、菜单、深色、大字体、横屏、安全区；
4. **性能基准**：先测量，再处理列表重组、主线程 IO 和动画；
5. **ImGui 删除**：删除真正的工作台渲染栈和三个 `.so`，保留独立编辑器/终端功能；
6. **引擎差异自动化**：持续补真实需要的 QuickJS API，并保持 Rhino 回归；
7. **文档归档**：统一当前文档，历史报告只作为指定提交的证据。
