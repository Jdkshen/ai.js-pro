...
打包时如果没有显式选签名（打包页的「自动签名」），就用**本机身份**：
`<脚本目录>/.keyStore/aijspro.keystore`，没有就顺手生成一份。

密钥库集中放在 `.keyStore/`（对齐 AutoX.js 的布局：它也是 `<脚本目录>/.keyStore/`，
创建时默认文件叫 `AutoX.keystore`），**一份身份服务所有打包应用** —— 产物目录不会每打一个
应用就多一个 p12，改了应用名也还能覆盖升级。口令记在应用私有偏好里，按密钥库路径记账。
产物是 **v1 + v2** 双签名。

旧版本在产物旁边放 `<应用名>-signing.p12`，升级后第一次打包会自动把它（连口令记录）
搬进 `.keyStore/` 继续用，不换身份、不用卸载重装。
...
| 选项 | 行为 |
| --- | --- |
| 自动签名 | 默认。用 `.keyStore/` 里那份本机身份（首次打包自动生成，之后固定复用） |
| 选择签名 | 用你自己的密钥库（`.jks` / `.keystore` / `.p12` / `.bks`），位置不限 |
| 新建签名 | 按你给的文件名在 `.keyStore/` 里生成一套新证书；同名文件已经存在时**直接载入那一份**（不覆盖，也不会又生成一套） |

卡片上始终有一行「**当前签名：…**」把即将使用的身份写出来：

- `本机专属 aijspro.keystore` —— 自动签名用的那份身份，已生成时直接显示证书主题
- `<你的证书主题>` —— 验证通过后显示为强调色
- `尚未验证，打包前请先点「验证密钥」` —— 填了口令但没点「验证密钥」时会走到这里（红色）；
  这时打包会被 `validate()` 拦住，不会静默换成别的证书

> 「新建签名」的默认文件名就是 `aijspro.keystore`，与自动签名用的是同一份身份：
> 点「生成」得到的是「已存在同名密钥库，已直接载入」（随后自动验证），而不是把用户堵在错误上。
> 想真再生成一套，把文件名改掉（例如 `mykey.keystore`）即可；同名即同一份身份，
> 否则每点一次「生成」就多出一套证书，之前签过的包反而再也覆盖升级不了。
...
要是口令都打不开那份密钥库（口令丢了、或者文件不是本机生成的），**不会把打包堵死**：
旧文件改名成 `<名字>.unreadable` 备份一份，再生成一套新身份继续打包，
...
> 写测试时注意：instrumented 测试跑在应用真实进程里，签名身份乂在真实脚本目录下的
> `.keyStore/`，直接写 `Pref` 会改到用户的真实数据 —— 必须备份并在 `finally` 里还原，
> 并用 `key_script_dir_path` 把脚本目录指向临时位置
> （曾因此把用户密钥库的口令覆盖掉，导致打包报「没有它的口令」）。打包成功后弹框还会多一行「签名：…」，
...
| `SigningOptions` | 打包页的选择逻辑（哪个模式用哪个签名器）、`.keyStore/` 目录、口令记账、新建时的文件名推导 |
| `AutoSigningIdentity` | 本机身份：`.keyStore/aijspro.keystore`；迁移旧布局，打不开就改名 `.unreadable` 后重新生成 |
