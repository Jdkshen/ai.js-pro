...
| `floaty` | 已接入 | `window(xmlOrConfig)` / `rawWindow` / `closeAll`；`window.<id>` 控件代理：`click()`/`click(fn)` / `longClick()`/`longClick(fn)` / `on("click"/"long_click"/"key"/"touch")` / `onKey` / `onTouch` / `getText` / `setText` / `setVisibility` / `requestFocus` / `attr(name[, value])` 与属性式读写（`view.text = 'x'`）；窗口级 `setSize` / `setPosition` / `getX` / `getY` / `getWidth` / `getHeight` / `findView` / `setAdjustEnabled` / `isAdjustEnabled` / `requestFocus` / `disableFocus` / `exitOnClose()`；不存在的控件 id 返回 undefined（与 Rhino 的 findView 回退一致）；触摸/按键事件经有界队列回到引擎线程；QuickJS 无 E4X，XML 使用字符串 |
