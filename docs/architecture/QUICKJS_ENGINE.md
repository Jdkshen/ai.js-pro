...
- **「先用 160 小模型 + 分块预处理」实测（K40，4 张 1080×2400 竖屏图，参考 = 方形 640 的检出）**：`detect(frame, { region })` 会把该区域 letterbox 进模型输入，所以分块/裁剪确实是「拿计算换有效分辨率」的手段，但账要算清：

  | 方案 | 有效缩放 | 耗时/张 | 相对 640 召回 | 匹配 IoU | 多余检出 |
  |---|---|---|---|---|---|
  | 方形 640（参考） | 0.2667 | 85.2ms | 100% | — | — |
  | 方形 160 整屏 | 0.0667 | **7.4ms** | **11%（2/19）** | 0.738 | 0 |
  | 方形 160 × 2 块 | 0.1333 | 15.6ms | 37%（7/19） | 0.697 | 8 |
  | 方形 160 × 3 块 | 0.148 | 23.0ms | 47%（9/19） | 0.815 | 5 |
  | **竖屏矩形 160×320 整屏** | 0.1333 | **13.2ms** | **42%（8/19）** | **0.840** | **3** |

  ⇒ 要点：① 分块能换回精度（11%→37%→47%），但**同样细节下竖屏矩形模型更快、更准、杂检更少**（分块把像素浪费在方块/长条的比例失配上，且目标被切在块边界会两边都检不到，还得多做跨块去重）；② 总像素量决定耗时——「k 倍细节 ≈ k² 倍算力」，分块不会凭空便宜；③ **真正划算的是「只裁关心的区域」**：固定区域（状态栏/技能栏/对话框）用 `detect(frame, { region })` 裁进去，等于免费把细节放大 N 倍、耗时还降到几分之一；④ 参考：区域为正方形时，方形 160 的细节 = 160/区域边长；区域为 1:2 时配 160×320 矩形模型最省（4 块 540×1080 ≈ 细节 0.296，比 640 整屏还高，代价 4×13.2ms）。
- **真机实时（截图 + 推理的端到端，脚本 `.artifacts/yolo-bench/rt-bench.js`）**：截图只要 2–3ms，瓶颈几乎全在推理。

  | 设备（截图分辨率） | 方形 160 | 竖屏 160×320 | 方形 640 |
  |---|---|---|---|
  | K40（1080×2400） | 12.2ms / **82 FPS** | 15.9ms / 63 FPS | 86.9ms / 11.5 FPS |
  | 新机（1280×2772） | 10.5ms / **95 FPS** | 11.3ms / 88 FPS | 65.9ms / 15.2 FPS |

  同一画面（未专门布景）下 160 方形与竖屏 160×320 均 0 检出，而 640 稳定检出 1 个目标——静态图上的漏检在实时场景同样成立。
  另外新机上 160×320（像素多一倍）只比方形 160 慢 15%：固定开销（截图/预处理）占比不小，小方形换来的速度优势没有静态推理差距那么大。
