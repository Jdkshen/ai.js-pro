...
| 悬浮窗跨引擎 / 生命周期 | 已接入 | 窗口注册表为**进程级**：`floaty.windowById(id)` / `floaty.getWindow(id)` / `floaty.exists(id)`；worker 线程（独立引擎）可按 id 操作主脚本创建的窗口；`win.on('attached'/'detached'/'close')`；引擎销毁只关闭自己创建的窗口 |
