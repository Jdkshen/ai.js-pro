...
| `floaty` | 已接入 | `window(xmlOrConfig)` / `rawWindow` / `closeAll`，支持创建配置 `{ x, y, visible: false }`（创建即定位，不再左上角闪现）；窗口级 `setSize` / `setPosition` / `getX` / `getY` / `getWidth` / `getHeight` / `findView` / `setAdjustEnabled` / `isAdjustEnabled` / `requestFocus` / `disableFocus` / `exitOnClose()` / **`show` / `hide` / `setVisibility` / `isShown` / `setContentVisible` / `setAlpha` / `getAlpha` / `setScale` / `setScaleX` / `setScaleY`**（显隐同步生效，隐藏 = `removeViewImmediate` 真正离屏；alpha/scale 直接作用于根 View，不重排布局）；`getX()/getY()` 立即返回设定值，`getX(true)/getRealX()` 读主线程 flush 后的生效值；`window.<id>` 控件代理：`click()`/`click(fn)` / `longClick()`/`longClick(fn)` / `on("click"/"long_click"/"key"/"touch")` / `onKey` / `onTouch` / `getText` / `setText` / `setVisibility` / `requestFocus` / `attr(name[, value])` 与属性式读写（`view.text = 'x'`）；不存在的控件 id 返回 undefined（与 Rhino 的 findView 回退一致）；触摸/按键事件经有界队列回到引擎线程；QuickJS 无 E4X，XML 使用字符串 |
